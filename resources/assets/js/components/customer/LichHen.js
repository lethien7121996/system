        ...
        <table className="table color-table primary-table" >
            ...
            {lichkhachhang.map(cd => ( 
              <tr key={cd.id} id={"lichkhachhangitem"+cd.id} data-itemcd={cd.id}>
                ...
                <td data-icd={cd.id} id={"tendichvu"+cd.id}>{this.renderDichVuDieuTri(cd.dichvu,cd.id)}</td>
                <td data-icd={cd.id} id={"tenbacsi"+cd.id}>{this.renderBacSi(cd.idbacsi,cd.id)}</td>
